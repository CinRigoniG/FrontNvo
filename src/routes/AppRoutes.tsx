...
import Sucursales from "../components/EmpresaSucursal/Sucursales";
import GrillaEmpresa from "../components/Grillas/GrillaEmpresa";

const AppRoutes = () => {
  const DondeEstamos = lazy(
    () => import("../components/DondeEstamos/DondeEstamos")
  );
  const Instrumentos = lazy(
    () => import("../components/Instrumentos/Instrumentos")
  );
  const DetalleInstrumentos = lazy(
    () => import("../components/DetalleInstrumentos/DetalleInstrumentos")
  );
    ...
    <Suspense fallback={<LoaderPage></LoaderPage>}>
      <Routes>
        ...
        <Route element={<RolUsuario rol={Roles.ADMIN} />}>
          ...
          <Route
            path="/formularioPedido/:idPedido"
            element={<FormularioPedido />}
          />
          <Route
            path="/formularioDetallePedido/:idDetPedido"
            element={<FormularioDetallPedido />}
          />
          <Route
            path="/detallePedido/:idPedido"
            element={<GrillaDetallePedido />}
          />
          <Route path="/grillaUsuarios" element={<GrillaUsuario />} />
          <Route
            path="/formularioUsuario/:idUsuario"
            element={<FormularioUsuario />}
          />
          <Route
            path="/formularioPersona/:usuarioId"
            element={<FormularioPersona />}
          />
        </Route>
        <Route path="/empresaSucursal" element={<GrillaEmpresa />} />

      <Route path="/sucursales/:idEmpresa" element={<Sucursales />} />
